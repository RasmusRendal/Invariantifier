...
def run_experiment(iterations, options, x_train, y_train, x_test, y_test, model, only_convolutional, filename):
    with open(filename, 'w') as f:
        ...
        for i in tqdm(range(10, iterations), desc=filename):
            ...
            res = checkSome(only_convolutional, x_test, y_test, model, training_samples, options)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--profile', dest='profile', action='store_true', help='Profile the experiments')
    parser.add_argument('--nobasic', dest='basic', action='store_false', help='Skip the basic experiment')
    parser.add_argument('--norep', dest='rep', action='store_false', help='Skip the representatives experiment')
    parser.add_argument('--noconv', dest='conv', action='store_false', help='Skip the convolution experiment')
    parser.add_argument('--iterations', type=int, default=501, nargs='?', help='How many iterations to run')
    ...
    if args.basic:
        ...
        options.post_init()
        cmd_string = "run_experiment(args.iterations, options, x_train, y_train, x_test, y_test, model, only_convolutional, experiment_dir + '/basic.csv')"
        if args.profile:
    ...
    if args.rep:
        ...
        options.post_init()
        cmd_string = "run_experiment(args.iterations, options, x_train, y_train, x_test, y_test, model, only_convolutional, experiment_dir + '/representatives.csv')"
        if args.profile:
    ...
    if args.conv:
        ...
        options.post_init()
        only_convolutional = split_network(model, options.convlayers)
        cmd_string = "run_experiment(args.iterations, options, x_train, y_train, x_test, y_test, model, only_convolutional, experiment_dir + '/convolution1.csv')"
        if args.profile:
